...
describe('Comptroller', function () {
  ...
  beforeEach(async function () {
    ...
    this.bitcoin = await Bitcoin.at(await this.comptroller.bitcoin());
  });
    ...
    describe('bootstrapping', function () {
      describe('on single call', function () {
        ...
        it('mints new Bitcoin tokens', async function () {
          expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(10100));
          expect(await this.bitcoin.balanceOf(this.comptroller.address)).to.be.bignumber.equal(new BN(0));
      ...
      describe('multiple calls', function () {
        ...
        it('mints new Bitcoin tokens', async function () {
          expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(10300));
          expect(await this.bitcoin.balanceOf(this.comptroller.address)).to.be.bignumber.equal(new BN(0));
          expect(await this.bitcoin.balanceOf(userAddress)).to.be.bignumber.equal(new BN(300));
        });
  ...
  describe('burnFromAccount', function () {
    beforeEach(async function () {
      ...
      await this.bitcoin.approve(this.comptroller.address, new BN(1000), {from: userAddress});
    });
    ...
    describe('on single call', function () {
      ...
      it('destroys Bitcoin tokens', async function () {
        expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(10900));
        expect(await this.bitcoin.balanceOf(this.comptroller.address)).to.be.bignumber.equal(new BN(0));
        expect(await this.bitcoin.balanceOf(userAddress)).to.be.bignumber.equal(new BN(900));
      });
    ...
    describe('multiple calls', function () {
      ...
      it('destroys Bitcoin tokens', async function () {
        expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(10700));
        expect(await this.bitcoin.balanceOf(this.comptroller.address)).to.be.bignumber.equal(new BN(0));
        expect(await this.bitcoin.balanceOf(userAddress)).to.be.bignumber.equal(new BN(700));
      });
  ...
  describe('redeemToAccount', function () {
    ...
    describe('on single call', function () {
      ...
      it('doesnt mint new Bitcoin tokens', async function () {
        expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(300));
        expect(await this.bitcoin.balanceOf(this.comptroller.address)).to.be.bignumber.equal(new BN(200));
        expect(await this.bitcoin.balanceOf(userAddress)).to.be.bignumber.equal(new BN(100));
      });
  ...
  describe('resetDebt', function () {
    ...
    describe('excess debt', function () {
      ...
      it('decreases debt', async function () {
        expect(await this.bitcoin.totalSupply()).to.be.bignumber.equal(new BN(10000));
